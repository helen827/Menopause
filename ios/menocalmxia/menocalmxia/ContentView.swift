            ...
            ZStack {
                ...
                TabView(selection: $selectedMode) {
                    ForEach(Array(modes.enumerated()), id: \.element.id) { index, mode in
                        BreathModeBackground(mode: mode)
                            .tag(index)
    ...
    private var bottomControls: some View {
        VStack(spacing: 16) {
            ...
            HStack(spacing: 16) {
                ...
                .accessibilityLabel(isRunning ? "暂停呼吸练习" : "继续呼吸练习")
...
private struct BreathModeBackground: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1 / 24)) { timeline in
            ...
            ZStack {
                LinearGradient(colors: mode.gradient, startPoint: .top, endPoint: .bottom)

                switch mode.kind {
                case .meadow:
                    MeadowBreathScene(time: time, mode: mode)
                case .sleepOrbit:
                    SleepOrbitBreathScene(time: time, mode: mode)
                case .coolMist:
                    CoolMistBreathScene(time: time, mode: mode)
                }
...
private struct MineTab: View {
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        ...
                        RecordCalendarCard(data: recordCalendarData)
    ...
    private func loadChatActivity() async {
        ...
        recordCalendarData = RecordCalendarData.currentEmpty(year: year, month: month)

        do {
            let response = try await AuthAPI.shared.chatActivity(
                year: year,
                month: month,
                tzOffsetMinutes: offsetMinutes
            )
            recordCalendarData = RecordCalendarData(activity: response.data)
        } catch {
            recordCalendarData = RecordCalendarData.currentEmpty(year: year, month: month)
        }
    }
    ...
    private var navItems: [MineNavItem] {
        [
            MineNavItem(title: "我的趋势报告", desc: "", icon: "chart.xyaxis.line", route: "trend_report"),
            MineNavItem(title: "就医清单", desc: "", icon: "cross.case.fill", route: "medical_checklist"),
            MineNavItem(title: "收藏内容", desc: "", icon: "bookmark.fill", route: "favorites"),
...
private struct RecordCalendarData {
    ...
    static func currentEmpty(year: Int? = nil, month: Int? = nil) -> RecordCalendarData {
        let calendar = Calendar.current
        let now = Date()
        return RecordCalendarData(
    ...
    init(activity: ChatActivityData) {
        self.year = activity.year
        self.month = activity.month
        self.scopeTitle = activity.scopeTitle
        self.checkinDays = Set(activity.checkinDays)
        self.meditationDays = Set(activity.meditationDays)
        self.consecutiveDays = activity.consecutiveDays
        self.practiceCount = activity.practiceCount
...
private struct RecordCalendarCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack(alignment: .firstTextBaseline) {
                Text("\(data.year)年\(data.month)月")
                    .font(.system(size: 18, weight: .black))
                    .foregroundStyle(Color(red: 0.40, green: 0.36, blue: 0.38))

                Spacer()

                Text("打卡 \(data.checkinCount) 天")
                    .font(.system(size: 15, weight: .bold))
                    .foregroundStyle(Color(red: 0.58, green: 0.52, blue: 0.56))
            }
            ...
            LazyVGrid(columns: columns, spacing: 8) {
                ...
                ForEach(Array(calendarCells.enumerated()), id: \.offset) { _, day in
                    if let day {
                        CalendarDayCell(
                            day: day,
                            hasCheckin: data.checkinDays.contains(day),
                            hasMeditation: data.meditationDays.contains(day)
                        )
                    } else {
                        Color.clear
                            .frame(height: 40)
            ...
            HStack(spacing: 16) {
                RecordMetricCard(value: data.consecutiveDays, label: "连续记录")
                RecordMetricCard(value: data.practiceCount, label: "练习次数")
            }
            .padding(.top, 4)

            FlowTags(tags: data.tags)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(22)
        .background(.white.opacity(0.82), in: RoundedRectangle(cornerRadius: 28, style: .continuous))
        ...
        .shadow(color: Color(red: 0.46, green: 0.34, blue: 0.41).opacity(0.10), radius: 20, y: 12)
    }

    private var calendarCells: [Int?] {
        var calendar = Calendar(identifier: .gregorian)
        calendar.locale = Locale(identifier: "zh_CN")
        guard let firstDay = DateComponents(calendar: calendar, year: data.year, month: data.month, day: 1).date,
              let range = calendar.range(of: .day, in: .month, for: firstDay) else {
            return []
        }
        let weekday = calendar.component(.weekday, from: firstDay)
        let leadingBlankCount = (weekday + 5) % 7
        return Array(repeating: nil, count: leadingBlankCount) + range.map { Optional($0) }
    }
}
...
private struct CalendarDayCell: View {
    ...
    var body: some View {
        Text("\(day)")
            .font(.system(size: 15, weight: .bold))
            .foregroundStyle(hasCheckin ? .white : Color(red: 0.42, green: 0.36, blue: 0.40))
            .frame(maxWidth: .infinity)
            .frame(height: 40)
            .background(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .fill(hasCheckin ? Color(red: 0.86, green: 0.61, blue: 0.63) : Color(red: 0.98, green: 0.94, blue: 0.96).opacity(0.92))
            )
            .overlay {
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .stroke(
                        hasMeditation ? Color(red: 0.78, green: 0.66, blue: 0.37) : Color(red: 0.91, green: 0.84, blue: 0.88),
                        lineWidth: hasMeditation ? 2 : 1
                    )
            }
    }
}
    ...
    var body: some View {
        LazyVGrid(columns: [GridItem(.adaptive(minimum: 74), spacing: 8)], alignment: .leading, spacing: 8) {
            ForEach(tags, id: \.self) { tag in
                Text(tag)
                    .font(.system(size: 14, weight: .bold))
                    .foregroundStyle(Color(red: 0.42, green: 0.36, blue: 0.40))
                    .frame(minHeight: 34)
                    .padding(.horizontal, 12)
                    .background(.white.opacity(0.70), in: Capsule())
                    .overlay {
                        Capsule()
                            .stroke(Color(red: 0.91, green: 0.84, blue: 0.88), lineWidth: 1)
                    }
            }
        }
    }
...
private struct MineNavRow: View {
        ...
        NavigationLink {
            ...
            HStack(spacing: 18) {
                Image(systemName: item.icon)
                    ...
                    .foregroundStyle(Color(red: 0.48, green: 0.42, blue: 0.46))
            ...
            .shadow(color: Color(red: 0.46, green: 0.34, blue: 0.41).opacity(0.08), radius: 16, y: 10)
        }
        .buttonStyle(.plain)
    }
    ...
    private var destination: some View {
        if item.route == "trend_report" {
            TrendReportView()
        } else if item.route == "medical_checklist" {
...
private struct TrendReportView: View {
    ...
    private var currentTrendCard: TrendReportCard? {
        currentReport?.trendCards.first(where: { $0.key == "sleep_trend" }) ?? currentReport?.trendCards.first
    }
    ...
    private var overviewCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(spacing: 12) {
                ...
                Text(currentReport?.overview?.title ?? "本周概况")
        ...
        .background(
            LinearGradient(
                colors: [
                    Color(red: 1.00, green: 0.82, blue: 0.86),
    ...
    private var frequentSymptomsCard: some View {
        TrendCard {
            VStack(alignment: .leading, spacing: 26) {
                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], alignment: .leading, spacing: 28) {
                    ForEach(currentSymptoms) { symptom in
                        HStack(spacing: 8) {
                            Image(systemName: symptom.icon)
                                .font(.system(size: 15, weight: .black))
                                .foregroundStyle(Color(red: 0.56, green: 0.49, blue: 0.53))

                            Text("\(symptom.label) (\(symptom.count)\(symptom.unit ?? "次"))")
    ...
    private var sleepTrendCard: some View {
        TrendCard {
            VStack(alignment: .leading, spacing: 24) {
                ...
                Text(currentTrendCard?.description ?? "柱状图表示睡眠问题严重程度")
                    .font(.system(size: 16, weight: .black))
                    .foregroundStyle(Color(red: 0.66, green: 0.60, blue: 0.63))
                ...
                ForEach(currentTriggers) { trigger in
                    HStack(alignment: .top, spacing: 22) {
                        ...
                        Text(trigger.text)
                            .font(.system(size: 18, weight: .black))
                            .lineSpacing(8)
                            .foregroundStyle(color(for: trigger.style))
                            .fixedSize(horizontal: false, vertical: true)
                    }
                }
            }
    ...
    private var nextStepCard: some View {
        TrendCard {
            VStack(alignment: .leading, spacing: 30) {
                ...
                ForEach(currentNextSteps) { step in
                    HStack(spacing: 22) {
...
private struct TrendCard<Content: View>: View {
    ...
    var body: some View {
        content
            ...
            .shadow(color: Color(red: 0.46, green: 0.34, blue: 0.41).opacity(0.09), radius: 20, y: 12)
    }
}
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(alignment: .firstTextBaseline) {
                Text(title)
                    .font(.system(size: 16, weight: .black))
                    .foregroundStyle(Color(red: 0.43, green: 0.36, blue: 0.40))
                ...
                if let source = quote?.source, !source.isEmpty {
                    if citationEnabled, let url = url(from: quote?.sourceUrl) {
                        Link("出处 \(source)", destination: url)
                            .font(.system(size: 13, weight: .bold))
                            .foregroundStyle(Color(red: 0.58, green: 0.50, blue: 0.55))
                            .lineLimit(1)
                    } else {
                        Text("出处 \(source)")
                            .font(.system(size: 13, weight: .bold))
            ...
            if let speaker = quote?.speaker, !speaker.isEmpty {
                if citationEnabled, let url = url(from: quote?.speakerUrl) {
                    Link("—— \(speaker)", destination: url)
                        .font(.system(size: 16, weight: .bold))
                        .foregroundStyle(Color(red: 0.48, green: 0.41, blue: 0.45))
                } else {
                    Text("—— \(speaker)")
                        .font(.system(size: 16, weight: .bold))
                        .foregroundStyle(Color(red: 0.48, green: 0.41, blue: 0.45))
